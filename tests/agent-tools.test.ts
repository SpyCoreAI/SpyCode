...
describe('read-only filesystem tools', () => {
  ...
  // ── H1: glob/grep read-containment (no escaping cwd) ──
  test('glob rejects patterns that escape cwd (../ + absolute) and never enumerates siblings', async () => {
    for (const pattern of ['../**/*', '../../**/*', '/etc/*', 'src/../../*', '..']) {
      const r = await dispatchTool('glob', { pattern }, ctx());
      expect(r.ok, pattern).toBe(false);
      expect(r.content, pattern).toMatch(/escapes the working directory/);
      // The sibling outside fixture (outsideDir/secret.txt) must never surface.
      expect(r.content, pattern).not.toContain('secret.txt');
    }
  });

  test('glob still resolves ordinary in-cwd patterns (regression)', async () => {
    const r = await dispatchTool('glob', { pattern: 'src/**/*.ts' }, ctx());
    expect(r.ok).toBe(true);
    expect(r.content).toContain('src/index.ts');
  });

  test('grep glob-scope cannot escape cwd to read sibling files (H1)', async () => {
    for (const glob of ['../**/*', '/**/*', '../../**/*']) {
      const r = await dispatchTool('grep', { pattern: 'top secret', glob }, ctx());
      expect(r.ok, glob).toBe(false);
      expect(r.content, glob).toMatch(/escapes the working directory/);
      expect(r.content, glob).not.toContain('top secret'); // outside content never read
    }
  });

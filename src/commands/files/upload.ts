...
import { isTrustedTokenHost, resolveApiUrl } from '../../lib/config.js';
    ...
    .action(
      async (
        ...
        try {
          ...
          // Attach the bearer token ONLY to trusted SpyCore hosts (+ localhost),
          // exactly like lib/api.ts and lib/sse.ts. A poisoned SPYCORE_API_URL /
          // --api-url pointing at an attacker host gets NO Authorization header,
          // so the token can't be exfiltrated on `files upload` either.

 ...
 * on); `resultInsideCwd` then post-filters matches as the hard guarantee.
 */
...
/**
 * True when a globby result (a path relative to `cwd`) stays inside `cwd`. The
 * hard read-side confinement backstop for glob/grep — mirrors `resolveInside`
 * so a crafted pattern can never surface a file outside the workspace even if
 * it slips past `assertContainedGlob`.
 */
function resultInsideCwd(cwd: string, relResult: string): boolean {
  const rel = relative(cwd, resolvePath(cwd, relResult));
...
const globTool: ToolDefinition = {
  ...
  async execute(args, ctx) {
    ...
    matches = matches.filter((f) => resultInsideCwd(ctx.cwd, f) && !isSecret(join(ctx.cwd, f)));
...
const grepTool: ToolDefinition = {
  ...
  async execute(args, ctx) {
    ...
    ).filter((f) => resultInsideCwd(ctx.cwd, f)); // never read outside cwd
